...
<body>
	<h3 style="font-size:2em;width:900px; height:60px; margin-left:300px; margin-right:auto;">주문 조회</h3>
			...
			<tbody align=center>
				...
				<c:choose>
					<c:when test="${empty newOrderList}">
						<tr>
							<td colspan="9" class="fixed" height="300px;"><strong>주문한 상품이 없습니다.</strong></td>
						</tr>
					</c:when>
					<c:otherwise>
						<c:forEach var="item" items="${newOrderList}" varStatus="i">
						<tr>
							<td><h2>${item.order_num}</h2></td>
							<td><h2>${item.id }</h2></td>
							<td><h2>${item.orderer_name}</h2></td>
							<td><h2>${item.company_name}</h2><br></td>
							<td><h2>${item.goods_title}</h2><br></td>
							<td><span>${item.order_goods_option}</span><br><span>${item.order_goods_qty}</span></td>
							<td><span>${item.order_price}원</span></td>
