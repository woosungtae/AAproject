...
function search_member(search_period){	
    ...
    formObj.method="get";
    formObj.action="/AA/admin/member/adminMemberMain.do";
    formObj.submit();
}
...
function fn_member_detail(order_id){
    ...
    document.body.appendChild(formObj); 
    formObj.method="post";
    formObj.action="/AA/admin/member/memberDetail.do";
    formObj.submit();
...
<body>
	<h3 style="font-size:2em;width:900px; height:60px; margin-left:300px; margin-right:auto;">등록 회원 조회</h3>
...
<table class="list_view">
   ...
   <c:choose>
     <c:when test="${empty member_list}">			
			<tr>
		       <td colspan=6 class="fixed" height="300px;">
	     ...
	     <c:forEach var="item" items="${member_list}" varStatus="item_num">
	            <tr >       
					<td width=10%>
					  ...
					  <a href="${pageContext.request.contextPath}/admin/member/memberDetail.do?id=${item.id}">
					     <h2>${item.id}</h2>
					  </a>
					</td>
					<td width=10%>
					...
					<td width=50%>
			            <c:choose>
			                <c:when test="${empty item.roadAdd}">
			                    <h2>등록 주소 없음</h2>
			                </c:when>
			                <c:otherwise>
			                    <h2>${item.roadAdd} ${item.jibunAdd} ${item.namujiAdd}</h2>
			                </c:otherwise>
			            </c:choose>
        			</td>
					<td width=10%>
         ...
         <tr>
             <td colspan=8 class="fixed">
		         ...
		         <c:if test="${chapter >1 && page==1 }">
		          <a href="${pageContext.request.contextPath}/admin/member/adminMemberMain.do?chapter=${chapter-1}&pageNum=${(section-1)*10 +1 }">&nbsp;pre &nbsp;</a>
		         </c:if>
		          <a href="${pageContext.request.contextPath}/admin/member/adminMemberMain.do?chapter=${chapter}&pageNum=${page}">${(section-1)*10 +page } </a>
		         <c:if test="${page ==10 }">
		          <a href="${pageContext.request.contextPath}/admin/member/adminMemberMain.do?chapter=${chapter+1}&pageNum=${section*10+1}">&nbsp; next</a>
		         </c:if> 
	      		</c:forEach> 
           </td>
        </tr>  		   
