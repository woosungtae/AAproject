	   ...
	   <c:choose>
	      <c:when test='${item.level > 1 }'>  
	         ...
	         <span style="font-size:12px;">[답변]</span>
                   <a class='cls1' href="${contextPath}/adboard/viewItem.do?adboard_num=${item.adboard_num}">${itme.adboard_title}</a>
	          </c:when>
	          <c:otherwise>
...
<c:if test="${memberInfo.status eq '관리자' }">
<a  class="write_btn2"  href="javascript:fn_itemForm('${isLogOn}','${contextPath}/adboard/itemForm.do', 
                                                    '${contextPath}/member/loginForm.do')"><input type="button" value="글쓰기" class="write_btn1"/></a></c:if>
