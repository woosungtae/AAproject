...
<body> 
<h3 style="font-size:2em;width:900px; height:60px; margin-left:300px; margin-right:auto;">1:1 문의</h3>
...
<c:choose>
  ...
  <c:when test="${csItemsList !=null }" >
    <c:forEach  var="csItem" items="${csItemsList }" varStatus="csItemNum" >
     <tr align="center" style="height:70px;" >
     <td width="5%" >${csItem.cs_num }</td>
	<td width="10%" >${csItem.id }</td>
	<td width="10%" >${csItem.cs_category }</td>
	  <td align='center'  width="35%" > 
	  <span style="padding-right:30px"></span>
	   <c:choose>
	      <c:when test='${csItem.level > 1 }'>  
	         <c:forEach begin="1" end="${csItem.level }" step="1">
	              <span style="padding-left:10px"></span>    
	         </c:forEach>
	         <span style="font-size:12px;">[답변]</span>
                   <a class='cls1' href="${contextPath}/cs/csViewItem.do?cs_num=${csItem.cs_num}">${csItem.cs_title}</a>
	          </c:when>
	          <c:otherwise>
	            <a class='cls1' href="${contextPath}/cs/csViewItem.do?cs_num=${csItem.cs_num}">${csItem.cs_title }</a>
	          </c:otherwise> 
	  ...
	  <td  width="10%" height="10%" style="font-size:0.9em;">${csItem.writeDate}</td> 
	  <td width="5%" height="10%" style="font-size:1em;">${csItem.viewCnt }</td>  
	</tr>
    </c:forEach>
     </c:when>
    </c:choose>
...
<a  class="write_btn2"  href="javascript:fn_itemForm('${isLogOn}','${contextPath}/cs/csItemForm.do', 
